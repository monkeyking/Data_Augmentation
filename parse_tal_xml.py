...
class ParseXml(object):
    ...
    def _parse_item(self, item):
        ...
        if item.find('bndbox'):
            ...
            if len(np.where(np.array(bbox) <= 0)[0])!=0:
                # 暂时忽略掉
                print("xml has <0 value")
                return True
...
def paser_text(text_path):
    class_list = []
    bbox_list = []
    with open(text_path, "r") as f:
        lines = f.readlines()
        for line in lines:
            class_list.append(int(line.split("\t")[0]))
            bbox = []
            bbox.append(int(line.split("\t")[1]))
            bbox.append(int(line.split("\t")[2]))
            bbox.append(int(line.split("\t")[3]))
            bbox.append(int(line.split("\t")[4]))
...
if __name__ =="__main__":
    print(paser_text("/home/tony/ocr/Data_Augmentation/1-4/train_bbbox_tmp/0_0.txt"))
